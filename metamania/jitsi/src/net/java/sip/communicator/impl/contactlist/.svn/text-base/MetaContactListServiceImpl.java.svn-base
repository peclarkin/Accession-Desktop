               ...
               ContactCapabilitiesListener
{
    ...
    /**
     * Lock for preventing changes to contacts from happening while we are
     * adding contacts to the underlying metacontact
     */
    ...
    {
        // Add the subscription listeners before adding the contact group to the
        // meta group in case the contact group changes while we are adding the
        // contacts.
        presenceOpSet
        ...
        if (rootProtoGroup != null)
        {
            ...
            // Take the lock while reading in the contacts to prevent any events
            // from the subscription listeners from affecting the root group
        ...
        implements SubscriptionListener
    {
        ...
        {
            // We need to synchronize this block to prevent trying to delete a
            // contact before it has been added.  If this happens, then the
            // deleted contact is added back resulting in a contact that the
            // user did not expect.
            synchronized (lock)
            {
                if (logger.isTraceEnabled())
