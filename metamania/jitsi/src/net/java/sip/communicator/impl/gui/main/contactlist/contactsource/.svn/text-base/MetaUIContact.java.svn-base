    ...
    extends UIContactImpl
{
    ...
    private void loadTooltip(final ExtendedTooltip tip)
    {
        ...
        while (i.hasNext())
        {
            ...
            if (provider.getSupportedOperationSets().containsKey(ppOpSetKey))
            {
                ...
                {
                    // The presence status is associated with the contact
                    // address, not other details belonging to the contact, so
                    // only show the status if we show the address.
                    Image bytes = ImageUtils.getBytesInImage(
                        protocolContact.getPresenceStatus().getStatusIcon());
                    ImageIcon protocolStatusIcon =
                        ImageLoader.getIndexedProtocolIcon(bytes, provider);
            ...
            OperationSetServerStoredContactInfo infoOpSet = provider
                .getOperationSet(OperationSetServerStoredContactInfo.class);

            if((infoOpSet != null) && (provider.isRegistered()))
            {
